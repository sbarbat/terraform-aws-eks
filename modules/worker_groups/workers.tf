...
resource "aws_autoscaling_group" "workers" {
  ...
  tags = concat(
    [
      {
        "key"                 = "Name"
        "value"               = "${var.cluster_name}-${lookup(each.value, "name", count.index)}-eks_asg"
...
resource "aws_launch_configuration" "workers" {
  for_each    = local.worker_groups_expanded
  name_prefix = "${var.cluster_name}-${lookup(each.value, "name", count.index)}"
...
resource "aws_iam_instance_profile" "workers" {
  for_each    = var.manage_worker_iam_resources && var.create_eks ? local.worker_groups_expanded : {}
  name_prefix = var.cluster_name
  role = lookup(
    each.value,
    "iam_role_id",
